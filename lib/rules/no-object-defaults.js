/**
 * @fileoverview Rule to flag when any property being declared in `View.defaultOptions` or
    ...
    create: function(context) {
        ...
        return {
            CallExpression: function(node) {
                ...
                if (!property || property.type !== 'Identifier' || property.name !== 'extend') {
                    return;
                }
                ...
                var defaults, defaultsPropertyName;
                node.arguments[0].properties.forEach(function(propObj) {
                    if (propObj.key.name === 'defaults' || propObj.key.name === 'defaultOptions') {
                ...
                defaults.properties.forEach(function(propObj) {
                    if (propObj.value.type === 'NewExpression') {
                        context.report(
                            propObj,
                            'a property of the `' + defaultsPropertyName + '` object must not be result of `new`'
                        );
                    }
                    if (propObj.value.type === 'ObjectExpression') {
                        context.report(
                            propObj,
                            'a property of the `' + defaultsPropertyName + '` object must not be an object'
                        );
                    }
                    if (propObj.value.type === 'ArrayExpression') {
                        context.report(
                            propObj,
                            'a property of the `' + defaultsPropertyName + '` object must not be an array'
                        );
                    }
                });
            }
