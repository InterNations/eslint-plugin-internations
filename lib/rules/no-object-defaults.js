      ...
      CallExpression(node) {
        ...
        defaults.properties.forEach(function(propObj) {
          ...
          if (propObj.value.type === 'NewExpression') {
            errorMessage = `a property of the \`${defaultsPropertyName}\` object must not be result of \`new\``;
            context.report(propObj, errorMessage);
          }
          if (propObj.value.type === 'ObjectExpression') {
            errorMessage = `a property of the \`${defaultsPropertyName}\` object must not be an object`;
            context.report(propObj, errorMessage);
          }
          if (propObj.value.type === 'ArrayExpression') {
            errorMessage = `a property of the \`${defaultsPropertyName}\` object must not be an array`;
            context.report(propObj, errorMessage);
          }
        });
      },
